        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <hr>
                    <form action="{{url('admin/admins/discount/update/'.$row->id)}}" method="POST" enctype="multipart/form-data">
                        {{csrf_field()}}

                        <input type="hidden" value="{{$user_id}}" name="user_id">
                        <div class="form-group row">
                            <label for="input-21" class="col-sm-2 col-form-label">{{trans('admin.discount')}} </label>
                            <div class="col-sm-10">
                                @php $input = "discount" @endphp
                                <input t class="form-control @error($input) is-invalid @enderror" id="input-21" value="{{$row->discount}}" name="{{$input}}">
                                @error($input)
