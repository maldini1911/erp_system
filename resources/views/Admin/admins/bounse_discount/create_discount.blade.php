        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <hr>
                    <form action="{{url('admin/admins/discounts/store')}}" method="POST" enctype="multipart/form-data">
                        {{csrf_field()}}

                        <input type="hidden" value="{{$user_id}}" name="user_id">
                        <div class="form-group row">
                            @php $input = "discount" @endphp
                            <label for="input-23" class="col-sm-2 col-form-label">{{trans('admin.discount')}}</label>
                            <div class="col-sm-10">
                                <input type="text" class="form-control @error($input) is-invalid @enderror" id="input-23" placeholder="Enter Your Discount" name="{{$input}}">
                                @error($input)
                        ...
                        <div class="form-group row">
                            <label for="input-22" class="col-sm-2 col-form-label">{{trans('admin.reason_discount')}}</label>
                            <div class="col-sm-10">
                                @php $input = "reason_discount" @endphp
                                <textarea type="text" rows="4" class="form-control @error($input) is-invalid @enderror" id="input-22" name="{{$input}}"></textarea>
                                @error($input)
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
